...
std::queue<int>& Character::getAction() {
...
void Character::strike(Character* target) {
  if (target->armor_ > 1 && target != nullptr) {
    target->armor_-=2;
  } else if (target->armor_ == 1 && target != nullptr){
    target->vitality_--;
    target->armor_--;
  } else {
    target->vitality_-=2;
...
void Character::throwTomato(Character *target)
{
    if (target != nullptr && target->armor_ == 0)
    {
        // target->armor -= 1;
        target->vitality_ -= 1;
    }
    else if (target != nullptr && target->armor_ > 0)
    {
        target->armor_ -= 1;
    }
    vitality_ += 1;
